document.addEventListener('DOMContentLoaded', function () {
    const userDetailsLink = document.getElementsByClassName('user-details');
    const body = document.getElementById('body');





    for (let i = 0 ; i < userDetailsLink.length; i++) {
        userDetailsLink[i].addEventListener('click', function (event) {
           const userId = userDetailsLink[i].getAttribute("data-user-id")


             body.innerHTML = " ";

            fillUserDetails(userId,body);



        });
    }


    function fillUserDetails(userId,body){

        fetch(`/users/details/${userId}`)
            .then(response => response.json())
            .then(userDetails => {
                const userDetailsDiv = document.getElementById('userDetails');
                let isItVip = "";

                let roleManege = ``;

                if (userDetails.itVip == true) {
                    isItVip = "Vip"
                    roleManege = `<a href="/users/make-user/${userDetails.id}" class="delete-button">Make it User</a>`

                }else {
                    isItVip = "User"
                    roleManege = `<a href="/users/make-vip/${userDetails.id}" class="delete-button">Make it Vip</a>`

                }


                body.innerHTML = ` 
                <div>
    <nav th:replace="~{fragments/navbar}"></nav>
</div>
<div class="container-fluid d-flex flex-column">

    <h1 class="text-center text-white mt-5" >Seller's Details</h1>
<div class="wrapper">
    <section id="car-details">
        <div class="info-section">
            <div class="car-header">

                <div class="img-container d-flex justify-content-center">
                    <img class="car-img" src="data:image/png;base64," alt='profile-image'/>

                    <img class="car-img" src="${userDetails.profilePictureUrl}" alt='profile-image'/>
                </div>

                <h1 class="d-block ">Full Name: ${userDetails.fullName} <th:block th:text=""></th:block></h1>


                <p class="description d-inline-block">Email: ${userDetails.email}<th:block th:text=""></th:block></p>
                <!-- user age -->
                <p class="description">Role : ${isItVip} <th:block th:text="?Vip:User"></th:block></p>

                <div class="buttons">
                    <a href="/offers/${userDetails.id}" class="edit-button">User's offers</a>
                    ${roleManege}
                </div>
            </div>
        </div>
    </section>
</div>
</div>



<div>
    <footer> </footer>
</div>
                `;


            })
            .catch(error => console.error('Error fetching user details:', error));
    }


});