document.addEventListener("DOMContentLoaded", function (){
    const currentUrl = window.location.href;
    console.log(cu)
    const userId = getUserIdFromUrl(currentUrl);
    fetch(`/users/details/${userId}`)
        .then(response => response.json())
        .then(userDetails => {
            const userDetailsDiv = document.getElementById('userDetails');
            userDetailsDiv.innerHTML = `
                     <div class="img-container d-flex justify-content-center">
                    <img class="car-img" src="data:image/png;base64," alt='profile-image'/>

                    <img class="car-img" th:src="${userDetails.getProfilePictureUrl()}" alt='profile-image'/>
                </div>

                <h1 class="d-block ">Full Name: <th:block th:text="${userDetails.getFullName()}"></th:block></h1>


                <p class="description d-inline-block">Email: <th:block th:text="${userDetails.getEmail()}"></th:block></p>
                <!-- user age -->
                <p class="description">Role: <th:block th:text="${userDetails.isItVip()}?Vip:User"></th:block></p>

                <div class="buttons">
                    <a th:href="@{/offers/{id}(id=*{userDetails.getId()})}" class="edit-button">User's offers</a>
                    <a href="{% url 'profile-delete' object.pk %}" class="delete-button">Delete</a>
                </div>
                    <!-- Add more details as needed -->
                `;
        })
        .catch(error => console.error('Error fetching user details:', error));
})

function getUserIdFromUrl(url) {
    const urlParts = url.split('/');
    const userIdIndex = urlParts.indexOf('details') + 1;

    if (userIdIndex < urlParts.length) {
        return urlParts[userIdIndex];
    } else {
        console.error('User ID not found in the URL');
        return null;
    }
}

